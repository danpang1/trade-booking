...
def _parse_iso(iso) -> datetime | None:
    """Parse a Postgres TIMESTAMPTZ / ISO 8601 string; None on bad input.

    Postgres TIMESTAMPTZ renders as '2026-05-19 08:42:00+00' or
    ISO 8601 — fromisoformat accepts both 'T' and ' ' separators
    in Py 3.11+ once normalized.
    """
    if not iso:
        return None
    try:
...
def fmt_dmy_hms(iso: str | None) -> str:
    """Return 'dd/mm/yyyy hh:mm:ss' from an ISO timestamp.

    '' on None / empty; the raw value on unparseable input so the
...
def cashflow_to_row(payload: dict, *, portfolios: Sequence[dict]) -> dict:
    ...
    return {
        "Input Date": fmt_dmy_hms(payload.get("first_effective_start")),
        ...
        "Trade Date": fmt_dmy_hms(payload.get("trade_date")),
        "Value Date": fmt_dmy_hms(payload.get("value_date")),
