"""Return every row version for one spot deal_ref, ordered oldest → newest.

Used by the audit-trail UI in Deal Enquiry. Reads
`{"deal_ref": "MFX00000001"}` from stdin. Writes
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            out = [spot_db.row_to_payload(cols, r) for r in rows]
