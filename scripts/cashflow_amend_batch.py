"""Bulk-amend cashflow rows in ONE transaction (all-or-nothing).

stdin JSON: {"rows": [<amend payload>, ...]}
Each payload is a full cashflow amend payload (same shape as cashflow_amend.py)
plus an optional "expected_effective_start" (ISO string) for optimistic
concurrency: if the live row's effective_start no longer matches, the whole
batch aborts (the row was changed by someone else since the client loaded it).
...
All-or-nothing: any failure (validation, stale row, bad loan mapping, DB error)
rolls back every row in the batch — nothing is written.
"""
...
def main() -> int:
    ...
    # Validate every payload up front, before opening a transaction.
        ...
        with conn:
            with conn.cursor() as cur:
                for p in rows:
                    ...
                    # Close the current live row. With expected_effective_start
                    # we also assert it's the exact version the client loaded
                    # (optimistic concurrency) — a mismatch means someone amended
                    # it in the meantime, so we abort the whole batch.
                    if expected:
                        cur.execute(
                    ...
                    # Replace loan mappings from _meta.loan_deal_refs (the client
                    # always sends the current set, changed or not, so unchanged
                    # rows keep their links). Same txn → rolls back with the rest.
                    meta = p.get("_meta") or {}
                    ...
                    mappings = loan_cashflow_map_db.set_mappings_for_cashflow(
                        ...
                        cashflow_type=p.get("cashflow_type"),
