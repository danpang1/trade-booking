"""Bulk-amend loan rows in ONE transaction (all-or-nothing).

stdin JSON: {"rows": [<amend payload>, ...]}
Each payload is a full loan amend payload (same shape as loan_amend.py) plus
an optional "expected_effective_start" (ISO string) for optimistic concurrency:
if the live row's effective_start no longer matches, the whole batch aborts
(the row was changed by someone else since the client loaded it).
...
All-or-nothing: any failure (validation, stale row, DB error) rolls back every
row in the batch — nothing is written. Mirrors cashflow_amend_batch.py; loans
carry no loan_cashflow_map mappings (that link lives on the cashflow side).
"""
...
def main() -> int:
    ...
    for i, p in enumerate(rows):
        if not isinstance(p, dict):
            ...
            return 3
        try:
            loan_db.validate_payload(p, mode="amend")
        ...
        with conn:
            with conn.cursor() as cur:
                for p in rows:
                    ...
                    out_rows.append(loan_db.row_to_payload(out_cols, cur.fetchone()))
