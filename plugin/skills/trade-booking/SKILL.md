...
1. **You never make HTTP calls.** Everything goes through the bundled CLI at `${CLAUDE_PLUGIN_ROOT}/bin/tokka-mo`. Always use that full path — never bare `tokka-mo`, because PATH setup is not guaranteed for non-engineer installs.
2. **Show a structured preview AND get explicit `y` before submitting.** Never submit on first parse. Never assume.
3. **Validate against the local refdata cache (`${CLAUDE_PLUGIN_ROOT}/bin/tokka-mo refdata refresh` if stale).** If a value isn't in the live set, STOP and ask — don't guess at the closest match.
...
6. **On `y`**: shell out to `${CLAUDE_PLUGIN_ROOT}/bin/tokka-mo book` (single) or `${CLAUDE_PLUGIN_ROOT}/bin/tokka-mo book-batch` (N≥2). Report draft IDs + the review URL.
