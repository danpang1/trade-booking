...
def _stub_refdata(monkeypatch):
    ...
    or readable inside the Docker test container. The validator silently skips
    enum checks when refdata loads return empty sets; that masked real
    validation gaps in CI."""
    monkeypatch.setattr(cashflow_db, "_load_counterparties_set",
                        lambda: {"0XRICK LIMITED", "BEBOP LTD"})
